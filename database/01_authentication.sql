-- Taken from http://postgrest.com/examples/users/
...
-- need the postgres extensions pgcrypto and uuid-ossp.
...
-- With the table in place we can make a helper to check passwords. It returns
-- the database role for a user if the email and password are correct.
...
-- they will use later to prove their identity. The tokens go in this table.
...
-- # Email Validation
--
-- This is similar to password resets. Once again we generate a
-- token. It differs in that there is a trigger to send validations
-- when a new login is added to the users table.
...
-- # Editing Own User
--
-- We'll construct a redacted view for users. It hides passwords
-- and shows only those users whose roles the currently logged
-- in user has db permission to access.
...
-- Using this view clients can see themeslves and any other users
-- with the right db roles. This view does not yet support inserts
-- or updates because not all the columns refer directly to
-- underlying columns. Nor do we want it to be auto-updatable because
-- it would allow an escalation of privileges. Someone could update
-- their own row and change their role to become more powerful.
-- 
-- We'll handle updates with a trigger, but we'll need a helper
-- function to prevent an escalation of privileges.
...
-- With the above function we can now make a safe trigger to
-- allow user updates.
...
-- Finally add a public function people can use to sign up. You
-- can hard code a default db role in it. It alters the underlying
...
as $$
  insert into basic_auth.users (email, pass, role) values
    (signup.email, signup.pass, 'users');
$$ language sql;

-- # Generating JWT
--
-- As mentioned at the start, clients authenticate with JWT.
-- PostgREST has a special convention to allow your sql functions
-- to return JWT. Any function that returns a type whose name ends
-- in jwt_claims will have its return value encoded. For instance,
-- let's make a login function which consults our users table.
--
...
--     }
--
-- Try decoding the token at jwt.io. (It was encoded with a
-- secret of secret which is the default.) To use this token
-- in a future API request include it in an Authorization
-- request header.
--
...
-- # Same-Role Users
--
-- You may not want a separate db role for every user. You
-- can distinguish one user from another in SQL by examining
-- the JWT claims which PostgREST makes available in the SQL
-- variable postgrest.claims. Here's a function to get the
...
-- role. You can modify login to set other claims as well if
-- they are useful for your other SQL functions to reference later.

-- # Permissions
--
-- Basic table-level permissions. We'll add an the authenticator
-- role which can't do anything itself other than switch into
-- other roles as directed by JWT.
create role anon;
create role users;
